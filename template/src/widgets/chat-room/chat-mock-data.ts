...
// Одинаковая переписка при каждом запуске: иначе поведение экрана нельзя
...
const LONG_TEXTS = [
  ...
  "Если коротко: одна модель данных и один рендерер на всех платформах — " +
    "@legendapp/list с общими размерами, темой и анимациями.",
...
const createCuratedMessages = (): MessageDto[] => {
  ...
  return [
    baseMessage({
      ...
      content: "Привет! Как тебе новый экран чата?",
    }),
    baseMessage({
      senderId: MOCK_CURRENT_USER_ID,
      createdAt: minutesAgo(40),
      content: "Летает! И одинаково на обеих платформах 🚀",
    }),
    baseMessage({
      senderId: MOCK_PEER.id,
 ...
 * Генерация детерминированная — экран всегда открывается одинаково.
 */
export const createMockMessages = (): MessageDto[] => {
  ...
  return (
    [...createBulkMessages(MOCK_MESSAGES_TOTAL - curated.length), ...curated]
      // Порядок задаёт хост: список рисует массив как есть. Курируемые
      // сообщения создаются вразнобой (minutesAgo 15, 42, 40, …), поэтому без
      // сортировки они встали бы не по времени.
      .sort(
        (a, b) =>
