...
/**
 * Разбор пропа `layout` в две части: метрики чата и метрики панели ввода.
 *
 * Проп остаётся плоским (тот же объект, что принимает нативная реализация) —
 * раскладывается он здесь. Раньше на это уходил список из ста имён ключей;
 * теперь ключ переопределяется просто потому, что он есть в дефолтах и пришёл
 * значением того же примитивного типа. Дописать метрику = дописать её в дефолт.
 */
