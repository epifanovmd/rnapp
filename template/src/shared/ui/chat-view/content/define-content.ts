 ...
 * не годятся: первый пересоздаёт нативные вью, ради переиспользования которых
...
export interface IChatContentType<K extends ChatContentTypeId> {
  /**
   * Идентификатор типа. Совпадает с `contentTypeID` нативной реализации —
   * обе платформы используют один словарь типов.
   */
  id: K;
  /**
