...
/**
 * Сырая высота клавиатуры — единственный низкоуровневый источник правды. Про
 * safe area, панели и списки не знает: только положение клавиатуры покадрово
 * на UI-потоке, включая интерактивное закрытие свайпом.
 *
 * `onStart` задаёт цель через `withTiming` — страховка на случай, если
 * покадровых `onMove` не будет (часть Android-конфигураций). Пришедший
 * `onMove` перебьёт анимацию присваиванием, второго источника не возникает.
 */
