 ...
 final class RNChatContainer: UIView {
     ...
     private var extraContentInsets: UIEdgeInsets = .zero
     ...
     @objc var insets: NSDictionary? {
         didSet {
             guard let insets else {
                 extraContentInsets = .zero
                 applyContentInsets()
                 return
             }
             extraContentInsets = UIEdgeInsets(
     ...
     private func parseConfiguration(_ dict: NSDictionary, base: ChatConfiguration) -> ChatConfiguration {
         ...
         if let layout = dict["layout"] as? [String: Any] {
             ...
             if let insets = RNChatParsing.insets(from: layout["contentInsets"]) { config.layout.contentInsets = insets }
             if let insets = RNChatParsing.insets(from: layout["bubbleContentInsets"]) { config.layout.bubbleContentInsets = insets }
             if let value = layout["avatarSize"] as? CGFloat { config.layout.avatarSize = value }
         ...
         if let colors = dict["colors"] as? [String: Any] {
             ...
             if let value = RNChatParsing.color(from: colors["groupTitleText"]) { config.colors.groupTitleText = value }
         ...
         if let fonts = dict["fonts"] as? [String: Any] {
             ...
             config.fonts.replyPreview = RNChatParsing.font(from: fonts["replyPreview"], fallback: config.fonts.replyPreview)
             config.fonts.replyPreviewSender = RNChatParsing.font(from: fonts["replyPreviewSender"], fallback: config.fonts.replyPreviewSender)
         }

         if let behavior = dict["behavior"] as? [String: Any] {
             if let value = behavior["showsAvatars"] as? Bool { config.behavior.showsAvatars = value }
             if let value = behavior["showsDateSeparators"] as? Bool { config.behavior.showsDateSeparators = value }
             ...
             if let value = behavior["scrollToCenterOnIdIndex"] as? Bool { config.behavior.scrollToCenterOnIdIndex = value }
         }
     ...
     private func applyContentInsets() {
         let base = hostViewController.configuration.layout.contentInsets
         let combined = UIEdgeInsets(
             top: base.top + extraContentInsets.top,
             left: base.left + extraContentInsets.left,
             bottom: base.bottom + extraContentInsets.bottom,
             right: base.right + extraContentInsets.right
         )
         hostViewController.chatViewController.setAdditionalInsets(combined)
     }
