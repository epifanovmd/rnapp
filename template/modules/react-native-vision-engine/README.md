...
end-to-end `[1, N, 6]` (v10/26). На Android кадр подаётся letterbox'ом
(пропорции сохраняются, поля 114/114/114 — как в обучении ultralytics)
с обратным пересчётом координат; на iOS Vision-пайплайн использует
`scaleFill` — конвенцию ultralytics-экспорта CoreML.

Слоты моделей — per-instance (`createVisionEngine` — по движку на
сканер), сами модели кэшируются нативно по имени на время жизни
приложения: повторные `loadDetector`/`loadObjectModel` дёшевы, ничего
не перечитывают и не перекомпилируют.
