...
class ChatSectionedAdapter(
    ...
    // Живой индекс сообщений: id → ChatMessage.
    // Используется для резолвинга reply — всегда актуальные данные.
     ...
     * Применяет DiffUtil для точечных изменений без лишних notifyDataSetChanged.
     * После dispatch принудительно сбрасывает alpha всех затронутых ViewHolder-ов
     * (защита от alpha=0f оставленного ContextMenuView на anchor).
     */
    fun submitSections(
        ...
        // Сбрасываем alpha — ContextMenuView скрывает anchor (alpha=0f).
        // При обновлении/удалении ViewHolder может сохранить неверный alpha.
        if (affectedIds.isNotEmpty()) {
            recyclerViewRef?.let { rv ->
                affectedIds.forEach { id ->
                    val pos = positionOfMessage(id)
                    if (pos >= 0) rv.findViewHolderForAdapterPosition(pos)
                        ?.itemView?.alpha = 1f
                }
            }
        }

        // Принудительный layout pass: при itemAnimator=null RecyclerView не всегда
        // детачит удалённые ViewHolder-ы в том же кадре — scrollBy(0,0) исправляет это.
        recyclerViewRef?.scrollBy(0, 0)
    }

    /** Позиция сообщения по id в плоском списке, -1 если не найдено */
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        // Гарантируем что alpha восстановлен при любом bind (переиспользование из пула)
    ...
    override fun onBindViewHolder(
        ...
        if (payloads.isEmpty()) {
            ...
            // Partial update — только данные, без пересоздания view
    ...
    override fun onViewRecycled(holder: RecyclerView.ViewHolder) {
        ...
        // Сбрасываем alpha при возврате в пул — защита от утечки состояния
        holder.itemView.alpha = 1f
    ...
    // ─── Reply resolution ─────────────────────────────────────────────────
    //
    // Резолвим reply ВСЕГДА из живого messageIndex — не из копии внутри ReplyInfo.
    // Это гарантирует что при редактировании/удалении оригинала цитата обновится
    // автоматически без дополнительных уведомлений.
    ...
    private fun resolveReply(message: ChatMessage): ResolvedReply? {
        ...
        return if (original != null) {
            ...
            // Оригинал удалён — используем snapshot из ReplyInfo
    ...
    private class ItemDiffCallback(
        ...
        override fun getChangePayload(oldPos: Int, newPos: Int): Any? = Unit // non-null = partial update
    }
}
